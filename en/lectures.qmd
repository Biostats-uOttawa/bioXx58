...
schedule <- schedule_df %>%
  ...
  ungroup() %>%
  mutate(
    Reading = ifelse(is.na(reading), NA, reading),
    Info = case_when(
    ...
    Topic = ifelse(is.na(topic), NA, topic),
    Assessment = ifelse(is.na(assessment), NA, assessment),
    Date = as.Date(
      case_when(
  ...
  select(Date, Topic, Info, Assessment, week, wk,session) %>%
  arrange(Date) %>%
...
schedule |>
  gt() |>
  cols_hide(c(week, wk,session)) |>
  fmt_markdown(columns = c("Topic", "Info", "Assessment")) |>
  fmt_date(
    columns = Date,
    date_style = "MMMd",
    locale = "en"
...
More importantly, notes have the commenting and annotating tools activated via <https://hypothes.is/>. If you create a free account , you can annotate and comment slides as well as share your annotations/comments with the public or via private or public groups.
