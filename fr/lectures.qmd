  ...
  mutate(
    ...
    Date = as.Date(
      case_when(
        ...
        day == "Lun" & week <= 5 ~ ymd(dat$date_start) + weeks(week - 1),
        day == "Lun" & week > 5 ~ ymd(dat$date_start) + weeks(week),
        day == "Mar" & week <= 5 ~ ymd(dat$date_start) + weeks(week - 1) + days(1),
        day == "Mar" & week > 5 ~ ymd(dat$date_start) + weeks(week) + days(1),
        day == "Ven" & week <= 5 ~ ymd(dat$date_start) + weeks(week - 1) + days(4),
        day == "Ven" & week > 5 ~ ymd(dat$date_start) + weeks(week) + days(4)
      )
    ),
    wk = ifelse((week %% 2) == 0, "a", ifelse(week < 20, "b", "c"))
  ) %>%
  select(Date, Topic, Reading, Assessment, week, wk) %>%
  arrange(Date) %>%
